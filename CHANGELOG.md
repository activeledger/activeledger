...
recovers, and the coverage to keep them honest.

### Fix
* **Network** : SPI sampled a stream and its `:stream` meta as two independent HTTP reads. A commit writes both in one leveldb batch, so a batch completing between those two responses handed back `state@43` paired with `meta@21` - a mismatched `meta._rev:state._rev` that SPI cannot repair afterwards, and the torn read the whole lock check exists to avoid. It is now one `allDocs({keys})`: one request, and one `driver.getMany()` inside the store, which takes a LevelDB snapshot and cannot straddle a batch write. `LevelMe.getMany()` needed fixing to match - it served whichever keys were warm from cache and fetched the rest from disk, which reintroduced the same torn pair because the cache probe is synchronous while the driver read is awaited. It now answers entirely from cache when every key is warm, and otherwise re-reads the whole set together.
* **Network** : A locked stream was reportable only when the lock was held by the ASKER'S OWN transaction and this node had voted against it. Only half of that carried any safety: a node that voted no never reaches `commit()`, so its copy cannot move - whose round is asking says nothing about whether the copy is stable. Two nodes each sitting on a rejected transaction for the same stream both refused to answer, for no reason beyond not having been asked by the right umid. It now looks up whichever umid holds the lock and asks whether this node voted against THEM.
* **Network** : A node that could not report vetoed every node that could. `spiConsensus` abstained the moment ANY node answered with the locked marker, however many others agreed - and on a busy stream some node is nearly always mid-transaction. Streams every node held byte-identically still logged NOWINNER on every round (218 times in two hours on one production stream, about a copy nothing disagreed about), and the same veto is why a genuinely diverged busy stream never healed: the condition that has to clear for repair is the condition traffic guarantees will not. Silence now counts as absence rather than objection. A revision must clear the threshold AND be held by more nodes than could not report, so nothing unseen can overturn it - three agreeing with one silent is decided, two agreeing with two silent is not, because if both silent nodes disagreed the real picture is a 2-2 fork.

### New
* **Network** : SPI now backfills the umid and replays its events itself. It rewrites a node's STATE; the umid of the transaction that produced that state, and the events its contract raised, are separate documents the node never wrote because it never ran that commit. Until now only activerestore's interagent backfilled them, driven by the 950 SPI raises - so the repair depended on a second process being alive, which turns out to be disabled outright on many nodes and has been observed dead for days in production. Neither shows as a fault: state converges, the node looks healthy, and only the event feed quietly stops. SPI already had everything it needed, so it now fetches from peers, adopts the majority copy and replays the events directly. The 950 is still written as the durable retry, and restore skips work already done. The origin path raises one too - not for its own failed transaction, which no peer holds, but for the umid named in the adopted `:stream` meta, which is the transaction actually missed.

### Fix
* **Activeledger CLI** : A node set up on a non-default port had its restore engine disabled. Setup switched off both autostarts whenever a port other than 5260 was given, which is right for activecore (it has its own port and would collide) but not for activerestore, which binds no port and reads no port config. The intent was to stop `--testnet`'s generated instances each running a restore engine; the cost landed on every real deployment instead. Testnet now passes `--disable-autostart` explicitly, and a port number no longer decides it.
* **Documentation** : Every package README embedded the logo from `activeledger.io`, which no longer serves the project - it 301s through a chain of unrelated third-party sites. The logo was dead everywhere, not only on npm, and every visitor to one of the 17 package pages was being redirected. All images now resolve; the eight IDE screenshots that exist nowhere in the repository are removed rather than left pointing at that chain.

### Changed
* **Tests** : 201 -> 290 unit tests, plus six live network checks. The SPI decision matrix (majorities in both directions, the ahead-minority contract stream, 2-2 forks, position ties, every shape of silence, non-default thresholds), the sampling endpoint, the storage and cache layer under concurrent load, the authorisation model with real keys, and restore recovering a document it lost along with the umid and events behind it. Nine of the matrix tests fail against the previous rule, so they discriminate rather than decorate. Live: convergence is now read off the nodes rather than inferred from a clean client response, a genuine same-position fork is proven to be left alone rather than guessed at, and a node is deliberately made to lose a document outright.
