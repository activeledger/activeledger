...
| `--stop` | Stop a running node. Reads the PID file and sends `SIGTERM` to each tracked top-level process (`activeledger`, `activestorage`, `activecore`, `activerestore`) in turn, then resets the PID file — the command itself returns quickly, no polling or wait built in. **Worth knowing before you rely on it**: the main process's own `SIGTERM` handler (`index.ts`) is just `process.exit()` — it doesn't explicitly kill the worker processes it forked (`network/process.js`, one per CPU core, plus the standby — see [network-internals.md](network-internals.md#the-worker-process-pool)), and Node doesn't do that automatically on a parent's exit. In practice this session found those workers do reliably die alongside the parent in normal use, but if you ever see `process.js`/`selfhost.js` instances still running in `ps aux` after `--stop`, that's this gap, not a sign anything is stuck — clean them up manually (they'll otherwise hold the storage engine's lock file and make the next start fail with "Instance already running"). |
...
The flip side: a node left on the exact default port (`5260`) keeps `autostart.restore` **on** (`autostart.core` now defaults to `false` regardless of port — see [core.md](core.md)). If you've explicitly re-enabled `autostart.core` on a default-port node, it binds `api.port` (default `5261`) — fine on its own, until you also enable the P2P transport (see [transport.md](transport.md)), whose server binds `main port + 1`, which for a default-port node is *also* `5261`. Enabling `p2pStreamServer` on a default-port node with `core` explicitly re-enabled will crash it with a port collision. **If you're running more than one node on the same host and plan to test P2P, keep every instance off the literal default port** — this was found the hard way while setting up a local benchmark for this doc set.
