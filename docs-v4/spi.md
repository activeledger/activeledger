...
- **You don't need to do anything about SPI as a client.** It's entirely a node-to-node concern; the origin either resolves it and the transaction eventually succeeds, or it exhausts its retries and the client gets a real failure back. There's no client-facing "SPI error" — `1200` is an internal vote-round signal, not something `ExternalInitalise` (see [transactions.md](transactions.md)) hands back directly.
