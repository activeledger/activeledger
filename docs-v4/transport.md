...
**If you've re-enabled `core` and also want P2P, watch out for this specific failure mode**: `autostart.core` defaults to `false` now (see [core.md](core.md)), so this doesn't bite out of the box — but if you've explicitly turned it back on for a node running on the literal default port (5260), it binds `api.port` (default `5261`), and the P2P server binds on `main port + 1`, which for a default-port node is *also* `5261`. Turning on `p2pStreamServer` on a default-port node with `core` re-enabled will crash it with a port collision. Either move that node off the default port, or leave `autostart.core` at its default `false` while using P2P.
